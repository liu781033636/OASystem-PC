...
//		}else {
	...
	if(userId!=null && rasmessage.getTitle()!=null){
			...
			try {
			rasmessage.setRper(userService.findById(User.class,userId));//设置收件人
			rasmessage.setSper(this.users());//设置发件人
			if(FileUpload.upload(uploadFile, uploadFileFileName, uploadFileContentType)==null){
				rasmessage.setFla1("0"); //空文件标识
			}else{
				System.out.println("上传成功");
				rasmessage.setAccessories(FileUpload.upload(uploadFile, uploadFileFileName, uploadFileContentType));//上传文件至服务器，返回文件地址
				rasmessage.setFla1(uploadFileFileName); //设置上传文件名
			}
		} catch (Exception e) {
			// TODO: handle exception
		}
		rasmessage.setRastime(new Date());//设置上传时间
		rasmessage.setRasstatus("0");//设置消息状态
		rasmessage.setFla2("0");//发件人删除状态
		rasmessage.setFla3("0");//收件人删除状态
		RasmessageService.saveAndRefresh(rasmessage);	//存入数据库
